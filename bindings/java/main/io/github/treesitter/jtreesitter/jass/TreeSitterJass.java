package io.github.treesitter.jtreesitter.jass;

import java.lang.foreign.*;

public final class TreeSitterJass {
    ...
    private static final TreeSitterJass INSTANCE = new TreeSitterJass();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterJass() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_jass");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-jass");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-jass");
